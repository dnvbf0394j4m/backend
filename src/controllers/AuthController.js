...
// ✅ DÙNG CỨNG, CHO CHẮC (sau này thích thì đổi sang env sau)
const ACCESS_EXPIRES = "15m";  // access token sống 15 phút
const REFRESH_EXPIRES = "30d"; // refresh token sống 30 ngày
...
export const login = async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign(accessPayload, process.env.JWT_SECRET, {
      expiresIn: ACCESS_EXPIRES, // ✅ LUÔN LÀ "15m"
    });
    ...
    const refreshToken = jwt.sign(
      refreshPayload,
      process.env.JWT_REFRESH_SECRET,
      { expiresIn: REFRESH_EXPIRES } // ✅ LUÔN LÀ "30d"
    );
    ...
    res.cookie("refreshToken", refreshToken, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production", // ✅ dev dùng http, prod dùng https
      sameSite: "strict",
      maxAge: 30 * 24 * 60 * 60 * 1000,
    });
